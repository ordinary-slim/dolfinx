...
def test_determine_point_ownership(dim, dtype):
    ...
    for x in Xs:
        for y in Ys:
            for z in Zs:
                ...
                counter += 1
    # Since the bounding boxes of the elements correspond to the elements,
    # the tree collisions are the exact collisions
    ...
    def check_po(src_points, po: PointOwnershipData, ownership_data, global_dest_owners):
        """
        ...
        global_owners: Rank who sent each point
        """
    ...
    check_po(points, po, ownership_data, owners)
    ...
    check_po(points, lpo, left_ownership_data, owners)
